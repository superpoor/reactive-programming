 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of a 1 number (10pts)`: Unit = {
    Assert.assertEquals(sum(List(3)), 3)
  }

  @Test def `sum of a empty list (10pts)`: Unit = {
    Assert.assertEquals(sum(List.empty), 0)
  }
  ...
  @Test def `max of 1 number`: Unit = {
    Assert.assertEquals(max(List(3)), 3)
  }

  @Test def `max of empty list`: Unit = {
    try {
      Assert.assertEquals(max(List.empty), 3)
